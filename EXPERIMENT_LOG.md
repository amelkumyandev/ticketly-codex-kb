...
| Auth Add-on | Add JWT authentication and authorization | 2026-05-12 11:11 +04:00 | 2026-05-12 11:25 +04:00 | `dotnet add ... JwtBearer`; `dotnet add ... Identity.Core`; `dotnet add ... System.IdentityModel.Tokens.Jwt`; `dotnet add ... Mvc.Testing`; `dotnet tool run dotnet-ef migrations add AddAuthenticationUsers ...`; `dotnet format`; `dotnet build`; `dotnet test`; `.\scripts\run-tests-with-coverage.ps1`; `powershell -ExecutionPolicy Bypass -File .\scripts\run-tests-with-coverage.ps1`; SonarQube token environment check; `.\scripts\estimate-token-burn.ps1`; `powershell -ExecutionPolicy Bypass -File .\scripts\estimate-token-burn.ps1 -AuthAddon` | Passed, 0 warnings, 0 errors | Passed, 17 tests | Passed with process-level execution-policy bypass; generated `TestResults\coverage.opencover.xml`; total line coverage 36.47% | Not run; no SonarQube token environment variable was available | Added JWT register/login, password hashing, user persistence, role policies, protected endpoints, auth tests, README auth docs, and token tracking. Token estimate: 734 input, 716 output, 1450 total. |
...
- Auth Add-on: Added a non-secret local default connection string in `appsettings.json` so the ASP.NET Core test host can start before replacing EF with the in-memory provider.
- Auth Add-on: Removed EF provider test registrations in the WebApplicationFactory setup so the test host uses only the in-memory provider.
...
- Auth Add-on: Exact platform token counts were not available in the local environment, so local token estimation was used.
- Auth Add-on: Direct PowerShell script execution remains blocked by local unsigned-script policy; coverage and token scripts were run with process-level execution-policy bypass.
- Auth Add-on: Build/test output still prints stale SonarScanner target messages for `.sonarqube\bin\targets\SonarQube.Integration.targets`, but commands pass with 0 warnings and 0 errors.
